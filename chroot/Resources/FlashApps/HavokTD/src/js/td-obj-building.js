	...
	// The properties and methods of the building object. Note that there are no arrays, objects, etc. in the attribute
 	...
 	var building_obj = {
		_init: function (cfg) {
			...
			/**
			 * Bullet type, can have the following types:
			 * 1: Ordinary bullets
		...
		toggleSelected: function () {
			...
			this.grid.hightLight(this.is_selected); // Highlight
 			var _this = this;
		 ...
		 * @param grid {Element} specify grid
		 */
...
//			return;
		...
		render: function () {
			...
			if (
				...
				ctx.fillStyle = "rgba(255, 136, 136, 0.15)";
				ctx.strokeStyle = "#C30101";
				ctx.beginPath();
			...
			if (this.type == "laser_gun" && this.target && this.target.is_valid) {
				...
				ctx.strokeStyle = "rgba(109, 255, 12, 0.5)";
				ctx.beginPath();
		...
		onEnter: function () {
			...
			var msg = "Building fortifications";
			if (this.map.is_main_map) {
	 ...
	 * 		{		
	 * 			Type: Construction types, optional values have
	 *				 "wall"
	 *				 "cannon"
	 *				 "LMG"
	 *				 "HMG"
	...
	// The properties and methods of the bullet object. Note that there are no arrays, objects, etc. in the attribute
	// Reference property, or the related properties of multiple instances conflict
 	var bullet_obj = {
		_init: function (cfg) {
			cfg = cfg || {};
	 ...
	 * 		Need to include at least the following:
	 * 		{
	 * 			x: Location of bullets
	 * 			Y: Where the bullets are sent
	 * 			Speed:
	 * 			Damage:
	 * 			Target: Goal, a Monster object
	 * 			Building: The building to which it belongs
	 *	}
	 * Bullet type, can have the following types:
	 * 1: Ordinary bullets
