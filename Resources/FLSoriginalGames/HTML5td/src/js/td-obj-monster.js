...
_TD.a.push(function (TD) {
	...
	// monster The properties and methods of the object. Note that there are no arrays, objects, etc. in the attribute
	...
	var monster_obj = {
		_init: function (cfg) {
			...
			this.grid = null; // Current lattice
			this.map = null;
			this.next_grid = null;
			this.way = [];
			this.toward = 2; // Default Face Downward
			this._dx = 0;
			this._dy = 0;

			this.is_blocked = false; // is the path of progress blocked?
		},
...
//		if (!this.map) return;
		...
		/**
		 *The monster was hit.
		 * @param building {Element} corresponding building (weapon)
		 * @param damage {Number} original damage value of this attack
		 */
		...
		/**
		 * Get Direction
		 * Which side of the current lattice is the next
		...
		/**
		 * Get to go to the next grid
		 */
		getNextGrid: function () {
			if (this.way.length == 0 ||
				Math.random() < 0.1 //There's a 1/10 chance of automatic road search.
			) {
				this.findWay();
			}
...
//			this.getToward(); // No use in this version for the time being.
		},

		/**
		 * Check if building buildings in the map (x, y) will block the current monster
		 * @param the x-coordinate of the MX {number} map
		 * y-coordinate @param my {number} map
		 * @return {Boolean}
		 */
		...
		/**
		 * The monster's path is blocked (surrounded by construction)
		 */
		beBlocked: function () {
		...
		step: function () {
			...
			if (!this.next_grid) {
				...
				/**
				 * If you still can't find the next available grid, it means the current monster is blocked.
 				 */
				if (!this.next_grid) {
					this.beBlocked();
					return;
...
//			}
	...
	/**
	 * The explosive effects of the monster's death.
	 */
	var explode_obj = {
		_init: function (cfg) {
			cfg = cfg || {};
	 ...
	 * 		R: Radius
	 * 		Color: RGB color, shaped as "#f98723"
	 * 		Scene: Scene objects
	 * 		Step_level:
	 * 		Render_level:
	 *
	 *		//The following options:
	 * 		Time: Duration, default is 1, unit is approximately seconds (depending on rendering, not very precise)
	 *	}
	 */
